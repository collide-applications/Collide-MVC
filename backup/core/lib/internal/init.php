 ...
 * @param	array	$cfg	application default config
 * @return	boolean	true on success false on error
 * @TODO    split in small functions
 */
if( !function_exists( 'initHook' ) ){
	function initHook( $url, $cfg ) {
		// prepare url
		$url = rtrim( trim( $url ), '/');
		...
		$viewClassName = 'View';
		if( class_exists( $cfg['default']['lib_prefix'] . $viewClassName ) ){
			$viewClassName = $cfg['default']['lib_prefix'] . $viewClassName;
		}

		// instantiate view
		$objView = new $viewClassName();

		// add default application config to view class
		$objView->setCfg( $cfg );

        // add controller name to view class
		$objView->setController( $controller );

		// add view to this controller
		$objController->setView( &$objView );
		...
		if( file_exists( APP_MODELS_PATH . $controller . EXT ) ){
			...
			$modelClassName = ucfirst( $controller ) .
							  $cfg['default']['model_sufix'];
			$objModel = new $modelClassName();

			// add model to this controller
 ...
 *
 * @global	array	$cfg		application config
